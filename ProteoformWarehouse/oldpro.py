...
if (len(sys.argv) < 2):
    combinedfilename = 'combined_file.xml'
    for q in range(0,numaccessions):
        ...
        if (not decoytest in accessionnum) and (not multipletest in accessionnum):
            ...
            if failure in str(result):
                ...
                combinedoutput = open(combinedfilename, "w")

                for x in range(0,len(finalfile)):
                    combinedoutput.write(finalfile[x] + "\n")
